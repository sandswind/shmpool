...
sms_init_statistics(struct base *root, struct base_config *cfg, int cfg_count)
{
    ...
    for (i=0; i<cfg_count; i++, cfg++) {
        b = &stat->buckets[i];
        if (cfg->type == SMS_RECORD_TYPE_ARRAY)
            a = (struct array *) ((char *)data + position);
        else
            r = (struct ring *) ((char *)data + position);
...
int 
sms_init(struct base *root, struct base_config *cfg, int cfg_count, int64_t total_size)
{
    int ret = 0;
    int64_t base_size = 0;
    int64_t static_size = 0;
...
sms_summary(struct base *root, FILE *out)
{
    ...
    for (i=0; i<stat->bucket_num; i++) {
        ...
        if (b->type == SMS_RECORD_TYPE_ARRAY) {
            a = (struct array *) ((char *)root + b->offset);
            ...
            r = (struct ring *) ((char *)root + b->offset);
            fprintf(out, ">>>>>> Bucket type [%s] :\n", "ring");
...
int
sms_push(struct base *root, uint32_t idx, void *elem, int32_t elem_size)
{
    int               ret = CMN_ERROR;
    struct statistics *stat = NULL;
    struct bucket     *b = NULL;
    ...
    if (root == NULL || idx > SMS_MAX_BUCKET_NUM)
        return CMN_PARAMETER;
...
int
sms_push_with_block(struct base *root, uint32_t idx, void *elem, int32_t elem_size)
{

}

int
sms_pop(struct base *root, uint32_t idx, void *elem)
{
    int               ret = CMN_ERROR;
    struct statistics *stat = NULL;
    struct bucket     *b = NULL;
...
int32_t
sms_pop_with_block(struct base *root, uint32_t idx, void *elem)
{
