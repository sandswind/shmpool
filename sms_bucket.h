...
#define SMS_RECORD_ALIGNMENT      64

#define BASE_STATISTICAL_AREA(main)      \
        ((char *) (main) + sizeof(struct base) + (sizeof(struct base) % SMS_RECORD_ALIGNMENT))

#define BASE_DATA_AREA(main)      \
        ((char *) (main) + sizeof(struct base) + (sizeof(struct base) % SMS_RECORD_ALIGNMENT) \
            + sizeof(struct statistics) + (sizeof(struct statistics) % SMS_RECORD_ALIGNMENT))
...
int sms_init(struct base *root, struct base_config *cfg, int cfg_count, int64_t total_size);
int sms_summary(struct base *root, FILE *out);
int sms_info(struct base *root);

int sms_push(struct base *root, uint32_t idx, void *elem, int32_t elem_size);
int sms_pop(struct base *root, uint32_t idx, void *elem);
